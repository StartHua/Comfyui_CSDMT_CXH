...
from .faceutils import faceplusplus as fpp
from .faceutils import dlibutils as dlib
...
def crop_image(image):
    ...
    image, face, crop_face = dlib.crop(image, face_on_image, up_ratio, down_ratio, width_ratio)
...
def makeup_transfer256(non_makeup_image, makeup_image,reszie,inittype):
    opts.resize_size = reszie
    opts.init_type = inittype
    makeup_model = CSD_MT(opts)
    ep0, total_it = makeup_model.resume(CSD_MT_PTH)
    makeup_model.eval()
